...
def assertResponse(r, threadID, code, index):
	ret = True
	...
	url = "           • TID : " + str(threadID) + " - #" + str(index).ljust(2, ' ') + " : " + str(
		r.request.method) + " "
	...
	def run(self):
		...
		for x in range(Mymax):
			try:
				...
				print("     " + str(self.threadID) + " - # " + str(index).ljust(3, ' ') + "RequestException : " + str(e))
				index += 1
...
def TESTS_42(testNum=0):
	...
	if testNum == 0 or index == int(testNum):
		...
		name = "Thread n°"
		for x in range(5):
			threads.append(Client((name + str(x + 1)), "GET", "http://localhost:8080", -1, 15))
		# 2. Lancement
		for x in threads:
			x.start()
		for x in threads:
	...
	if testNum == 0 or index == int(testNum):
		...
		name = "Thread n°"
		for x in range(20):
			threads.append(Client((name + str(x + 1)), "GET", "http://localhost:8080", -1, 200))
		# 2. Lancement
		for x in threads:
			x.start()
		for x in threads:
