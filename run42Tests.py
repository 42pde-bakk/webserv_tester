...
def resBodySize(r, size):
	if r.text and str(len(r.text)) >= size and str(r.headers["Content-Length"]) == size:
		return True
	else:
		return False
...
def moreAsserts(r, assertLevel, *args):
	...
	if AssertTypes.BODY_CONTAIN_ASSERT in assertLevel and bodyContain(r, args[0][indexArgsUsed]):
		ret = True
		indexArgsUsed += 1
	if AssertTypes.FILE_CONTAIN_ASSERT in assertLevel and fileContain(r, args[0][indexArgsUsed], args[0][indexArgsUsed + 1]):
		ret = True
		indexArgsUsed += 2
	if AssertTypes.RES_HD_CONTAIN_ASSERT in assertLevel and resHeadersKeyVal(r, args[0][indexArgsUsed], args[0][indexArgsUsed + 1]):
		ret = True
		indexArgsUsed += 2
	if AssertTypes.RES_BODY_SIZE_ASSERT in assertLevel and resBodySize(r, args[0][indexArgsUsed]):
		ret = True
		indexArgsUsed += 1
...
def assertResponse(r, code, index, assertLevel=[], *args):
	...
	if ret and r.status_code == code:
		...
		info = Bcolors.FAIL + "KO" + Bcolors.ENDC + " - " + str(
			r.status_code) + " - Should have been received : " + str(code)
...
def TESTS_42(testNum=0):
	...
	index += 1
	if follow and testNum != 0 and index >= testNum:
		chainUp = True
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/", headers=hd)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		# The last \n seems to be send in another request ... idk why, may be to trool us but it may raise an error of
		# invalid request
		r = requests.post("http://localhost:8080/", headers=hd, data=payload)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.head("http://localhost:8080/", headers=hd)
		assertResponse(r, 405, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory", headers=hd)
		assertResponse(r, 200, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/youpi.bad_extension", headers=hd)
		assertResponse(r, 200, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/youpi.bla", headers=hd)
		assertResponse(r, 200, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/oulalala", headers=hd)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/nop", headers=hd)
		assertResponse(r, 200, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/nop/other.pouic", headers=hd)
		assertResponse(r, 200, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/nop/other.pouac", headers=hd)
		assertResponse(r, 404, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/Yeah", headers=hd)
		assertResponse(r, 404, index)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		r = requests.get("http://localhost:8080/directory/Yeah/not_happy.bad_extension", headers=hd)
		assertResponse(r, 200, index)
	...
	if follow and testNum != 0 and index >= testNum:
		chainUp = True
	if chainUp or testNum == 0 or index == int(testNum):
		if os.path.exists("www/test42/file_should_exist_after"):
			os.remove("www/test42/file_should_exist_after")
		payload = "3e8\r\n"
		payload += "e" * 1000
		payload += "\r\n"
		payload += "0\r\n\r\n"
		...
		r = requests.put("http://localhost:8080/put_test/file_should_exist_after", headers=hd, data=payload)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		# The last \n seems to be send in another request ... idk why, may be to trool. It may raise an error of
		# invalid request
		r = requests.put("http://localhost:8080/put_test/file_should_exist_after", headers=hd, data=payload)
		assertResponse(r, 204, index)
		# No longer needed :
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		payload += "\r\n"
		payload += "0\r\n\r\n"
		r = requests.post("http://localhost:8080/directory/youpi.bla", headers=hd, data=payload)
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		payload += "\r\n"
		payload += "0\r\n\r\n"
		r = requests.post("http://localhost:8080/directory/youpla.bla", headers=hd, data=payload)
		assertResponse(r, 200, index, [AssertTypes.RES_BODY_SIZE_ASSERT], "100000000")
	...
	if chainUp or testNum == 0 or index == int(testNum):
		...
		payload = "0\r\n\r\n"
		r = requests.post("http://localhost:8080/post_body", headers=hd, data=payload)
		assertResponse(r, 200, index)
